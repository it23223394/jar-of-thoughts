...
import { BarChart2, Search, Moon, Sun, Palette, Plus, X } from 'lucide-react'
...
function FirePit() {
  return (
    <svg width="56" height="64" viewBox="0 0 56 64" fill="none">
      <defs><radialGradient id="fire-glow" cx="50%" cy="80%" r="60%"><stop offset="0%" stopColor="#ff6b00" stopOpacity="0.4"/><stop offset="100%" stopColor="#ff6b00" stopOpacity="0"/></radialGradient></defs>
      ...
      <ellipse cx="28" cy="44" rx="18" ry="6" fill="url(#fire-glow)"/>
      <motion.path d="M20 44 Q18 34 22 28 Q24 22 20 18 Q26 20 26 30 Q28 24 30 18 Q34 24 30 32 Q34 28 36 22 Q40 30 36 40 Q34 44 28 46 Q22 44 20 44Z" fill="#ff6b00" animate={{scaleY:[1,1.08,0.96,1.05,1],scaleX:[1,0.97,1.03,0.98,1]}} transition={{duration:1.4,repeat:Infinity,ease:'easeInOut'}} style={{transformOrigin:'28px 44px'}}/>
...
function SteelBox({ isOpen }) {
  return (
    <svg width="64" height="60" viewBox="0 0 64 60" fill="none">
      <defs>
        <linearGradient id="steel-body2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#9CA3AF"/><stop offset="100%" stopColor="#4B5563"/></linearGradient>
        <linearGradient id="steel-lid2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#D1D5DB"/><stop offset="100%" stopColor="#9CA3AF"/></linearGradient>
      </defs>
      <rect x="4" y="28" width="56" height="30" rx="3" fill="url(#steel-body2)" stroke="#374151" strokeWidth="1"/>
      <circle cx="10" cy="34" r="2" fill="#6B7280"/><circle cx="54" cy="34" r="2" fill="#6B7280"/>
      <circle cx="10" cy="52" r="2" fill="#6B7280"/><circle cx="54" cy="52" r="2" fill="#6B7280"/>
      ...
      <motion.g animate={{rotateX:isOpen?-70:0}} transition={{type:'spring',stiffness:200,damping:20}} style={{transformOrigin:'32px 28px'}}>
        <rect x="4" y="14" width="56" height="16" rx="3" fill="url(#steel-lid2)" stroke="#374151" strokeWidth="1"/>
    ...
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      <motion.div initial={{y:'100%'}} animate={{y:0}} exit={{y:'100%'}} transition={{type:'spring',stiffness:300,damping:30}}
        ...
        <div style={{marginBottom:'16px'}}>
          <div style={{fontSize:'12px',color:t.panelMuted,marginBottom:'8px',textTransform:'uppercase',letterSpacing:'0.05em'}}>Name</div>
        ...
        <div style={{marginBottom:'16px'}}>
          <div style={{fontSize:'12px',color:t.panelMuted,marginBottom:'8px',textTransform:'uppercase',letterSpacing:'0.05em'}}>Emoji</div>
          <div style={{display:'flex',flexWrap:'wrap',gap:'8px'}}>
            {JAR_EMOJIS.map(e=>(
              <button key={e} onClick={()=>setEmoji(e)}
                style={{width:'36px',height:'36px',borderRadius:'8px',border:`2px solid ${emoji===e?t.writeBtnBg:t.panelBorder}`,background:emoji===e?t.writeBtnBg+'22':'transparent',fontSize:'18px',cursor:'pointer'}}>
                {e}
              </button>
            ))}
          </div>
        ...
        <div style={{marginBottom:'20px'}}>
          <div style={{fontSize:'12px',color:t.panelMuted,marginBottom:'8px',textTransform:'uppercase',letterSpacing:'0.05em'}}>Color</div>
          <div style={{display:'flex',flexWrap:'wrap',gap:'8px'}}>
            {JAR_COLORS.map(c=>(
        ...
        <motion.button whileTap={{scale:0.97}} onClick={()=>{if(name.trim()){onAdd({name:name.trim(),emoji,color});onClose()}}}
          disabled={!name.trim()}
        ...
        {lockEnabled&&(
          <div style={{display:'flex',flexWrap:'wrap',gap:'6px',marginBottom:'14px'}}>
            {[['1m','1 month'],['3m','3 months'],['6m','6 months'],['1y','1 year']].map(([val,label])=>(
              <button key={val} onClick={()=>setLockOption(val)} style={{padding:'5px 12px',borderRadius:'16px',fontSize:'12px',cursor:'pointer',border:`1px solid ${lockOption===val?t.writeBtnBg:t.panelBorder}`,background:lockOption===val?t.writeBtnBg:'transparent',color:lockOption===val?'#fff':t.panelMuted}}>{label}</button>
            ))}
          </div>
        )}
  ...
  return (
    <div style={{minHeight:'100vh',background:t.pageBg,display:'flex',flexDirection:'column',position:'relative',overflow:'hidden',transition:'background 0.4s ease'}}>
      ...
      <div style={{display:'flex',gap:'8px',padding:'12px 16px 0',overflowX:'auto'}}>
        <div style={{background:t.panelBg,border:`1px solid ${t.panelBorder}`,borderRadius:'12px',padding:'8px 12px',flexShrink:0,display:'flex',gap:'6px',alignItems:'center'}}>
        ...
        {thoughts.filter(t=>t.isLocked&&new Date(t.unlockAt)>new Date()).length>0&&(
          <div style={{background:t.writeBtnBg+'22',border:`1px solid ${t.writeBtnBg}44`,borderRadius:'12px',padding:'8px 12px',flexShrink:0}}>
            <span style={{fontSize:'11px',color:t.writeBtnBg,fontWeight:600}}>🔒 {thoughts.filter(t=>t.isLocked&&new Date(t.unlockAt)>new Date()).length} locked</span>
          </div>
        )}
        <button onClick={()=>setShowAddJar(true)}
          style={{background:t.writeBtnBg,border:'none',borderRadius:'12px',padding:'8px 12px',flexShrink:0,display:'flex',alignItems:'center',gap:'4px',cursor:'pointer',color:'#fff',fontSize:'11px',fontWeight:600}}>
          <Plus size={12}/> New jar
        </button>
      </div>

      {/* Hint */}
      <div style={{textAlign:'center',fontSize:'10px',color:t.hintText,letterSpacing:'0.08em',textTransform:'uppercase',padding:'10px 0 0',transition:'color 0.3s'}}>
        tap to open · double-tap to write
      </div>
          ...
          <div style={{display:'flex',justifyContent:'center',alignItems:'flex-end',gap:'4px',flexWrap:'wrap'}}>
            {jars.map((jar,i)=>(
              <motion.div key={jar.id} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:i*0.06}}
                ...
                <div onClick={()=>handleJarClick(jar)} onDoubleClick={()=>handleJarDoubleClick(jar)} style={{cursor:'pointer',position:'relative'}}>
                ...
                <div style={{fontSize:'9px',color:openJarId===jar.id?t.jarLabelActive:t.jarLabel,textAlign:'center',fontWeight:openJarId===jar.id?600:400,transition:'color 0.2s'}}>
                  {jar.emoji}
                </div>
              </motion.div>
            ))}
